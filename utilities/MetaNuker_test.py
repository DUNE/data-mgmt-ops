...
def ActualNuke(myfid=None,verbose=False,fix=False,level=-1):
    print("in ActualNuke fix= %s, myfid= %s ",fix,myfid)
    if fix:
        print (indent[level],"this is where you would code the removal of ",myfid)
        mc_client.retire_file(fid=myfid, retire=True)
    ...
    for parent in parents:
        ...
        if {"fid":myfid} in siblings:
            ...
            if fix:
                ...
                mc_client.update_file(fid=myfid,parents=[],replace=True)
            else:
...
if __name__ == '__main__':
    ...
    if nukeme:
        status = NukeMe(myfid=myfid,verbose=True,fix=True,level=0)
